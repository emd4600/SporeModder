...
public class ModelAnim {
	...
	public ModelAnim(ModelAnim other) {
	...
	public ArgScriptCommand toCommand() {
		...
		if (curveVary != 0) command.putOption(new ArgScriptOption(Float.toString(curveVary)));
		if (speedScale != 0) command.putOption(new ArgScriptOption(Float.toString(speedScale)));
		if (channelId != 0) command.putOption(new ArgScriptOption(Integer.toString(channelId)));
	...
	public void parse(ArgScriptCommand command) throws ArgScriptException {
		...
		{ ArgScriptOption option = command.getOption("channelId"); if (option != null) channelId = Byte.parseByte(option.getSingleArgument()); }
	...
	// For Syntax Highlighting
		...
		public static String[] getOptionTags() {
			return new String[] {
				"vary", "speedScale", "channelId", "sustain", "single", "loop", "length"
			};
		}
